...
function* handleGetExercise(action) {
  try {
    ...
    if (status === 200) {
      ...
      yield put({
        type: actions.DASHBOARD2_GET_EXERCISE_SUCCESS,
        exercises: exercises,
      });
    } else {
      yield put({
        type: actions.DASHBOARD2_GET_EXERCISE_ERROR,
        error: 'Unknown Error',
      });
    }
  } catch (error) {
...
export default all([
  takeLatest(actions.DASHBOARD2_GET_EXERCISE_REQUEST, handleGetExercise),
]);
